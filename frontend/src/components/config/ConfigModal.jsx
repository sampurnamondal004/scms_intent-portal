...
export default function ConfigModal({ action, title, row, fields, onClose }) {
    ...
    <div className="fixed inset-0 z-40 grid place-items-center bg-black/30 px-4" onClick={onClose}>
      <section className="w-full max-w-2xl rounded border border-slate-300 bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="grid gap-3 p-5 sm:grid-cols-2">
          {fields.map((field, index) => (
            <label className="field-label" key={field}>
              {field}
              <input className="scms-input" defaultValue={row?.[index] || ''} readOnly={action === 'View'} />
            </label>
          ))}
        </div>
        <div className="flex justify-end gap-2 border-t border-slate-200 px-5 py-3">
          ...
          {action !== 'View' && (
            <button className="rounded bg-[#0f8b55] px-4 py-2 text-sm font-bold text-white" type="button" onClick={onClose}>
              Save
            </button>
          )}
        </div>
